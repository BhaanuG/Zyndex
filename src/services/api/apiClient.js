...
export const testDatabaseConnection = async () => {
  try {
    ...
    if (response.ok) {
      ...
      if (data.database_time) {
        console.log('[API Test] Database timestamp:', new Date(data.database_time).toLocaleString());
      }
      return data;
    } else {
      console.error('[API Test] ❌ Database connection failed:', data.error);
      return data;
    }
  } catch (error) {
    console.error('[API Test] ❌ Error connecting to database:', error.message);
    throw error;
  }
};
